(: find sentences with 12-18 words :)
declare variable $db := ( 
"grc-com"
) ;
...
declare function local:selectsents($db){
  for $sentence in db:open($db)//*:sentence[not(*:word[@insertion_id] or *:word[@artificial])]
where 12 <= count($sentence/*:word[not(@lemma="punc1")]) and count($sentence/*:word[not(@lemma="punc1")]) <= 18
return element tr { 
